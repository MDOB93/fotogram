...
function getImgHtml(index) {
    return `<div class="img-container" id="imgContainer${index}" tabindex="0" onclick="openDialog(${index})" onkeydown="handleImgKeydown(event, ${index})" aria-labelledby="imgContainer${index}"><img src="${images[index]}" alt="${ImgTitel[index]}" ></div>`
}
...
function handleImgKeydown(event, index) {
    ...
    // Default behaviour of Space on focused elements
    if (event.key === ' ') {
...
function focusNextImage(index) {
    ...
    const nextEl = document.getElementById(`imgContainer${nextIndex}`);
    nextEl.focus();
}

function focusPrevImage(index) {
    const prevIndex = (index - 1 + images.length) % images.length;
    const prevEl = document.getElementById(`imgContainer${prevIndex}`);
    prevEl.focus();
}
...
dialogRef.addEventListener('keydown', (event) => {
    if (event.key === 'ArrowRight') {
        ...
        imgForward(currentIndex);
    }

    if (event.key === 'ArrowLeft') {
        event.preventDefault();
        const currentIndex = getCurrentDialogIndex();
        imgBackwards(currentIndex);
    }
});
